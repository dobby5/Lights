          ...
          if (light == 0) {
            for (int pixel = 0; pixel < dividedLightsArray[0]; pixel++)
            {
              if (pixel < dividedLightsArray[0] - transitionLeds / 2) {
                strip->SetPixelColor(pixel + pixelOffset, convFloat(lights[light].currentColors));
              } else {
                strip->SetPixelColor(pixel + pixelOffset, blending(lights[0].currentColors, lights[1].currentColors, pixel + 1 - (dividedLightsArray[0] - transitionLeds / 2 )));
              }
            }
          }
          else {
            for (int pixel = 0; pixel < dividedLightsArray[light]; pixel++)
            {
              ...
              if (pixel < transitionLeds / 2 && pixelOffset < pixel) {
                ...
                strip->SetPixelColor(pixel + pixelSum + transitionLeds + pixelOffset, blending( lights[light].currentColors, lights[light + 1].currentColors, pixel + transitionLeds / 2 - dividedLightsArray[light]));
              }
              else
              {
                if (pixel <= pixelOffset && light == lightsCount - 1) {
                  if (pixel > pixelOffset - transitionLeds / 2 - 1 && transitionLeds != 0) {
...
bool loadConfig() {
  ...
  pixelOffset = (uint16_t) json["pixelOffset"];
  for (uint16_t i = 0; i < lightsCount; i++) {
...
void setup() {
  ...
  server.on("/", []() {
    if (server.arg("section").toInt() == 1) {
      ...
      pixelOffset = server.arg("pixeloffset").toInt();
    ...
    for (uint8_t light = 0; light < lightsCount; light++) {
      if (lightsCount > 1) {
        if (light == 0) {
          ...
          {
            if (pixel < dividedLightsArray[0] - transitionLeds / 2) {
              strip->SetPixelColor(pixel + pixelOffset, convInt(lights[light].currentColors));
            } else {
              strip->SetPixelColor(pixel + pixelOffset, blendingEntert(lights[0].currentColors, lights[1].currentColors, pixel + 1 - (dividedLightsArray[0] - transitionLeds / 2 )));
            }
          }
        }
        else {
          for (int pixel = 0; pixel < dividedLightsArray[light]; pixel++)
          {
            ...
            if (pixel < transitionLeds / 2 && pixelOffset < pixel) {
              ...
              strip->SetPixelColor(pixel + pixelSum + transitionLeds + pixelOffset, blendingEntert( lights[light].currentColors, lights[light + 1].currentColors, pixel + transitionLeds / 2 - dividedLightsArray[light]));
            }
            else
            {
              if (pixel <= pixelOffset && light == lightsCount - 1) {
                if (pixel > pixelOffset - transitionLeds / 2 - 1 && transitionLeds != 0) {
                  ...
                  strip->SetPixelColor(pixel - 1, convInt(lights[light].currentColors));
                }
              }
